...
import { useCeleryEvaluation } from '../api/celeryApi';
...
const CeleryCodeEvaluation: React.FC = () => {
  const {
    submissionFile,
    idealFile,
    error,
  ...
  const handleGenerateSummaryFeedback = async () => {
    ...
    try {
      ...
      const highCount = result.function_results.filter((f: any) => f.quality === 'high').length;
      const mediumCount = result.function_results.filter((f: any) => f.quality === 'medium').length;
      const lowCount = result.function_results.filter((f: any) => f.quality === 'low').length;
      const poorCount = result.function_results.filter((f: any) => f.quality === 'poor').length;
      const missingCount = result.function_results.filter((f: any) => f.quality === 'missing').length;
  ...
  return (
    <div className="code-evaluation">
      <h1>🚀 Code Evaluation (Celery - Parallel Processing)</h1>
      <p className="description">
        Upload ZIP files containing Python code to evaluate student submissions against ideal solutions using parallel processing.
      </p>
      ...
      <div className="evaluation-form">
        ...
        <div className="form-actions">
          ...
          >
            {isEvaluating ? 'Evaluating...' : 'Evaluate Code (Celery)'}
          </button>
        ...
        <div className="evaluation-results">
            ...
            <div className="function-results">
              ...
              <div className="results-table">
                <table>
                  ...
                  <tbody>
                    {result.function_results.map((func: any, index: number) => {
          ...
          {result.extra_functions.length > 0 && (
            <div className="extra-functions">
              ...
              <ul>
                {result.extra_functions.map((func: any, index: number) => (
          ...
          {result.missing_functions.length > 0 && (
            <div className="missing-functions">
              ...
              <ul>
                {result.missing_functions.map((func: any, index: number) => (
                  <li key={index}>{func}</li>
                ))}
              </ul>
            </div>
...
export default CeleryCodeEvaluation;
